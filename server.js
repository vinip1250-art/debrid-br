...
// ============================================================
// Novo Upstream BASE: Torrentio com filtros PT
const UPSTREAM_BASE = "https://torrentio.strem.fun/providers=nyaasi,tokyotosho,anidex,comando,bludv,micoleaodublado|language=portuguese";
const BRAZUCA_MANIFEST = "https://94c8cb9f702d-brazuca-torrents.baby-beamup.club/manifest.json"; // Segunda Fonte
const DEFAULT_NAME = "Brazuca Torrents PT"; 
...
app.get('/addon/manifest.json', async (req, res) => {
    ...
    try {
        ...
        // Buscamos o manifesto do Torrentio como base
        ...
        manifest.id = `community.torrentio.proxy.${idSuffix}`; // Novo ID
        manifest.name = customName; 
        manifest.description = `Wrapper TorBox - Conteúdo PT/BR`;
        manifest.logo = customLogo;
        manifest.version = PROJECT_VERSION; 
        
        // Remove catálogos extras para evitar lentidão
...
// ============================================================
// 3. ROTA REDIRECIONADORA (STREAMS/CATÁLOGOS)
// ============================================================
...
app.get('/addon/*', async (req, res) => {
    ...
    // 1. Lógica de Streams ou Catálogos
    if (originalPath.startsWith('/stream/') || originalPath.startsWith('/catalog/')) {
        res.setHeader('Content-Type', 'application/json');
        
        try {
            const response = await axios.get(upstreamUrl);
            // Retorna o JSON do Torrentio diretamente
            return res.json(response.data);

        } catch (error) {
            console.error("Stream/Catalog Fetch Error:", error.message);
            return res.status(404).json({ streams: [] }); 
        }
    }
    
    // 2. Lógica de Redirecionamento (Meta, etc.)
...
<body class="min-h-screen flex items-center justify-center p-4 bg-black">
    ...
    <div class="w-full max-w-lg card rounded-2xl shadow-2xl p-6 border border-gray-800 relative">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-gray-500 text-xs mt-1 uppercase tracking-widest">GERADOR STREMTHRU V${PROJECT_VERSION} (TORBOX ONLY)</p>
        </div>
            ...
            <div class="space-y-3">
                ...
                <div class="bg-[#161616] p-3 rounded border border-gray-800">
                    <label class="flex items-center gap-3">
                        <span class="text-sm font-bold text-gray-300">✔ Torrentio (Base) + Brazuca</span>
                    </label>
                    <p class="text-[10px] text-gray-500 mt-1 ml-1">Fontes otimizadas para conteúdo em português/BR.</p>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-[#1a1a1a] p-4 rounded-xl border border-gray-800">
                    <div class="flex items-center gap-2 mb-4">
                        <input type="checkbox" id="use_tb" class="w-5 h-5 accent-purple-600 cursor-pointer" checked onchange="validate()">
                ...
                <!-- REAL DEBRID (REMOVIDO DA INTERFACE) -->
                <div class="bg-red-900/30 p-2 rounded-lg text-center text-xs">
                    Real-Debrid removido por bloqueio de IP.
                </div>
            </div>
    ...
    <script>
        ...
        const BRAZUCA_MANIFEST_URL = "${BRAZUCA_MANIFEST}";
        const TORRENTIO_PT_URL = "${UPSTREAM_BASE}/manifest.json"; // Agora nosso novo proxy base
        ...
        function generate() {
            ...
            // O nosso proxy para o Brazuca será a PRIMEIRA fonte
            const myMirrorUrl = window.location.origin + "/addon/manifest.json" + proxyParams + "&t=" + Date.now();

            let config = { upstreams: [], stores: [] };
            
            // 1. Adiciona o Torrentio como proxy base (para o nome e streams)
            config.upstreams.push({ u: myMirrorUrl });
            
            // 2. Adiciona o Brazuca Torrents como segunda fonte
...
app.get('/addon/manifest.json', async (req, res) => {
    ...
    try {
        ...
        // CORRIGIDO: Agora aponta para o Brazuca original para pegar o conteúdo
        const response = await axios.get(`${BRAZUCA_MANIFEST}`);
        const manifest = response.data;
...
app.get('/addon/stream/:type/:id.json', async (req, res) => {
    ...
    try {
        // CORRIGIDO: Retorna o 404 para o StremThru buscar no próximo Upstream.
        return res.status(404).json({ streams: [] });
